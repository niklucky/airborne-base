...
class PostgresMapper extends BaseMapper {
  ...
  connect(cb) {
    const { Client } = require('pg'); // eslint-disable-line global-require
    ...
    const conn = new Client(connection);
    conn.connect();
    conn.on('error', (err) => {
  ...
  get(params) {
    if (params instanceof Object === false) {
      throw new Error('[Fatal] PostgresMapper: you have to provide params for get()');
    }
      ...
      try {
        ...
        return this.exec(query, (error, rows) => {
          ...
          if (rows === undefined) {
            ...
            resolve(this.buildCollection(JSON.parse(data)));
          }
        });
      } catch (e) {
        return reject(e);
  ...
  create(params, payload) {
    if (payload instanceof Object === false) {
      throw new Error('[Fatal] PostgresMapper: you have to provide data for create()');
    }
    return new Promise((resolve, reject) => {
      try {
  ...
  update(params, payload) {
    if (params instanceof Object === false) {
      throw new Error('[Fatal] PostgresMapper error: you have to provide params for update(). You cannot update whole table.');
    }
    if (payload instanceof Object === false) {
      throw new Error('[Fatal] PostgresMapper error: you have to provide payload for update(). Nothing to update.');
    }
    return new Promise((resolve, reject) => {
      try {
        const model = new this.Model(payload);
  ...
  del(params) {
    if (params instanceof Object === false) {
      throw new Error('[Fatal] PostgresMapper error: you have to provide params for del(). You cannot delete the whole table.');
    }
    return new Promise((resolve, reject) => {
      try {
